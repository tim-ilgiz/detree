...
namespace IdentityServer.Controllers
{
    ...
    {
        private readonly Dictionary<string, AuthorityIssuer> _issuers;

        public AuthorityController(ILogger<AuthorityController> logger, IAccountRepository authenticationRepository)
        {
            var cancellationToken = new CancellationToken();
            //For testing purpose
            authenticationRepository.InsertAccount("vee", "qwertyui", "+66821113334", out var guid, cancellationToken);
        ...
        public IActionResult Account(string authority, [FromBody] AuthorityModel model)
        {
            ...
            if (principle?.Identity?.IsAuthenticated == true)
                try
                {
                    ...
                    var verifyResult = _issuers["owner"]
                        .Verify(authority, claimsIdentity.Claims.ToArray(), model.Payload);
                    if (verifyResult.Authority == null)
                        return Ok(new {auth_token = verifyResult.Token});
                    return Ok(new
                    {
                        verify_token = verifyResult.Token, authority = verifyResult.Authority,
